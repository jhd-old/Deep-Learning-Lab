...
#################
# Methods for the normal-to-depth block
#################


def get_pixelgrid(b, h, w, cuda=True):
    """
    Calculates a pixelgrid for given batchsize, height and width

    :param b: batchsize
    :param h: height
    :param w: width
    :param cuda: use cuda (gpu)
    :return: generated pixelgrid
    :rtype: torch tensor (bxhxw)
    """
...
def normals_to_disp3(inv_K, normal, cuda=True):
    """
    Calculates disparity form given normal vectors using pinhole camera model.

    :param inv_K: inverse instrinsic camera matrix
...
def normal_to_depth(inv_K, normal, min_depth, max_depth):
    ...
    :param inv_K: inverted intrinsic matrix (Torch tensor)
    :param normal: normals vector (Torch tensor)
